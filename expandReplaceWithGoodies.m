function p = expandReplaceWithGoodies(attemptcount,newThresh)

global numFact numDose
global SelectedVectors TopVectors GoodiesPool TabuList Xselect GoodiesUsedRWG

TopVectors = [TopVectors SelectedVectors]; % ReplaceWithGoodies 16
GoodiesUsedRWG = 0;
...
scoreThresh = topScore*(1-(newThresh/100));
...
while p < r && n > 0
    ...
    %   Generate a pool of vectors from the newRoot vector by +/- attemptcount
    %   dose level for a single factor.
    AddToGP = repmat(transpose(newRoot),numFact*2,1);
    for i=1:numFact
        AddToGP(i,i) = AddToGP(i,i)+attemptcount;
        AddToGP(numFact+i,i) = AddToGP(numFact+i,i)-attemptcount;
