...
%
% Inputs (*=default):
%  X   = CellOfStrings, with rows to be sorted. Must be 2D (matrix).
...
%      = StringToken, 'descend'/'ascend'* sort direction selection.
%  xpr = StringToken, regular expression to detect numeric substrings.
...
%
% Outputs:
%  Y   = CellOfStrings, input <X> with the rows sorted as per <col>.
%  ndx = NumericVector, of size M*1. Row indices such that Y = X(ndx,:).
%
...
assert(ismatrix(X),'First input <X> must have size M*N (matrix).');
%
if nargin<3
	xpr = '\d+';
end
...
elseif isnumeric(col)
	...
	assert(all(floor(col)==col)&&all(abs(col)<=n)&&all(col~=0),...
		'Second input <col> must be a vector of column indices into first input <X>.')
	varargin{end+1} = [];
